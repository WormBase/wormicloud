...
export const getCounters = store => getCloudState(store) ? getCloudState(store).counters.filter(counter => counter.active).map(counter => {return {text: counter.text, value: counter.value}}) : [];
